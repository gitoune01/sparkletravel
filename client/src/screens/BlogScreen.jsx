...
export const BlogScreen = () => {
  ...
  const cat = useEffect(() => {
    let pageItems = '1';
    dispatch(getBlogByCat({ category, pageItems }));
    window.scroll(0, 0);
  }, [category, dispatch]);
          ...
          {blogs.map((blog) => (
            ...
            >
              <Stack direction={{ base: 'column', lg: 'row' }} spacing={'7'}>
                <Image
                  src={blog.image}
                  minW={{ lg: '400px'}} maxH= '280px' 
                  loading={<Spinner  />} fit="cover"
                />
                <Flex direction="column">
                  ...
                  <Flex width={'full'} py="2">
                    ...
                    <Text>
                      Category: <Link pl='1' as={ReactLink} to={`/blog/${blog.category}`}>{category?.charAt(0).toUpperCase() + category.slice(1)}</Link>
                    </Text>
                    <Spacer />
                    <Link as={ReactLink} to={`/${blog._id}`}>Read More...</Link>
                  </Flex>
                  <Divider />
                </Flex>
              </Stack>
